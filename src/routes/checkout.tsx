...
export const Route = createFileRoute('/checkout')({
  ...
  head: () => ({ meta: [{ title: 'Checkout | GW Vacation Homes' }] }),
})
...
function CheckoutPage() {
  ...
  async function handleStripeCheckout() {
    if (!email.trim() || items.length === 0) {
      toast.error('Email and at least one listing required')
      return
    }
  ...
  if (items.length === 0) {
    return (
      <StorefrontLayout>
        <PageHero title="Checkout" backLabel="Browse properties" backTo="/" contained />
