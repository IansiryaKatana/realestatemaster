...
function SearchPage() {
  ...
  usePageMeta({
    title: q ? `Search: ${q} | GW Vacation Homes` : 'Search | GW Vacation Homes',
    description: q ? `Search results for “${q}” at GW Vacation Homes.` : 'Search listings across GW Vacation Homes.',
  ...
  return (
    <StorefrontLayout>
      <PageHero title="Search" subtitle={q ? `${total} result(s) for “${q}”` : 'Find villas, apartments, and homes across Dubai'} />
      ...
      <div className="px-6 py-10 md:px-14">
        <form method="get" action="/search" className="relative mx-auto mb-10 max-w-xl">
          <Input
            ...
            placeholder="Search by area, property type, or reference…"
        ...
        {q.trim() === '' ? (
          <p className="text-center text-muted">Enter a keyword to search listings.</p>
        ) : loading ? (
