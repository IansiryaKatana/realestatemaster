...
export function StockAlertForm({ productId, variantId, disabled }: StockAlertFormProps) {
  ...
  async function submit() {
    ...
    if (!sb) {
      toast.error('Availability alerts require a live connection')
      return
    }
    setLoading(true)
    ...
    toast.success('We will email you when this listing is available again')
    setEmail('')
  }

  return (
    <div className="mt-4 rounded-lg border border-[#e8e0d4] p-4">
      <p className="text-sm font-semibold text-text-brown">Notify me when available again</p>
