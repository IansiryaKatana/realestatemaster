...
function RelatedProductsCarousel({ products }: { products: Product[] }) {
  ...
  return (
    <div className="mt-10 md:hidden">
      ...
      {products.length > 1 ? (
        <div className="flex justify-center gap-1.5 pt-5" role="tablist" aria-label="Related listings">
          {products.map((item, i) => (
            <button
              key={item.id}
              type="button"
      ...
      <SectionContainer>
        <div className="mx-auto max-w-[420px] text-center">
          ...
          <p className="mt-2 text-[11px] leading-relaxed text-muted">
            More listings picked for you from the same category
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <p className="mt-10 text-center text-sm text-muted">Could not load related listings.</p>
        ) : (
          <>
