...
import { CheckCircle2, Circle, CreditCard, Loader2, Save } from 'lucide-react'
...
export function AdminIntegrations() {
  const { refetchCms, snapshot } = useCms()
  const [publishableKey, setPublishableKey] = useState('')
  const [secretKey, setSecretKey] = useState('')
  const [webhookSecret, setWebhookSecret] = useState('')
  const [mode, setMode] = useState<'test' | 'live'>('test')
  const [enabled, setEnabled] = useState(false)
  const [checkoutMode, setCheckoutMode] = useState<'quote' | 'stripe'>('quote')
  ...
  const refresh = useCallback(async () => {
    ...
    for (const row of data ?? []) {
      ...
      if (row.key === 'checkout_mode') setCheckoutMode(row.value === 'stripe' ? 'stripe' : 'quote')
    }
    setLoading(false)
  }, [])
  ...
  async function saveSecretKey() {
    ...
    if ((body as { auto_enabled?: boolean }).auto_enabled) {
      setEnabled(true)
  ...
  const setupSteps = [
    { label: 'Stripe enabled', done: enabled },
    { label: 'Checkout mode set to Stripe', done: checkoutMode === 'stripe' },
    { label: 'Publishable key saved', done: Boolean(publishableKey.trim() || snapshot.siteSettings.stripe_publishable_key?.trim()) },
    { label: 'Secret key + webhook secret saved in admin', done: enabled && checkoutMode === 'stripe' },
    { label: 'Webhook endpoint registered in Stripe Dashboard', done: false },
    { label: 'Edge functions deployed (create-checkout-session, stripe-webhook, create-property-checkout-session)', done: false },
  ]
      ...
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--admin-muted)]">Online payments for rent, deposits, commissions, and fees</p>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-[var(--admin-border)] p-4">
          ...
          <p className="mt-1 text-xs text-[var(--admin-muted)]">
            Saving both the secret key and webhook secret together auto-enables Stripe and switches checkout to Stripe mode.
          </p>
            ...
            {setupSteps.map((step) => (
              <li key={step.label} className="flex items-start gap-2 text-sm">
                {step.done ? (
                  ...
                  <Circle className="mt-0.5 h-4 w-4 shrink-0 text-[var(--admin-muted)]" />
                )}
                <span className={step.done ? 'text-green-800' : 'text-[var(--admin-muted)]'}>{step.label}</span>
              </li>
            ))}
          </ul>
          <p className="mt-3 text-xs text-[var(--admin-muted)]">
            Stripe webhook URL: <code className="rounded bg-[var(--admin-primary-muted)] px-1">{webhookUrl}</code>
            <br />
            Subscribe to: <code className="rounded bg-[var(--admin-primary-muted)] px-1">checkout.session.completed</code>,{' '}
            <code className="rounded bg-[var(--admin-primary-muted)] px-1">checkout.session.expired</code>,{' '}
            <code className="rounded bg-[var(--admin-primary-muted)] px-1">charge.refunded</code>
          </p>
        </div>
      </div>
    </div>
