...
type Row = Database['public']['Tables']['lifestyle_cards']['Row']
type Layout = 'large' | 'small' | 'wide'

type FormState = { title: string; cta_label: string; cta_url: string; image_url: string; layout: Layout; sort_order: string; is_active: boolean }

const emptyForm = (): FormState => ({ title: '', cta_label: '', cta_url: '', image_url: '', layout: 'small', sort_order: '0', is_active: true })

export function AdminLifestyleCards() {
  ...
  function openEdit(row: Row) {
    setEditing(row)
    setForm({ title: row.title, cta_label: row.cta_label ?? '', cta_url: row.cta_url ?? '', image_url: row.image_url ?? '', layout: row.layout as Layout, sort_order: String(row.sort_order), is_active: row.is_active })
    setModalOpen(true)
  }
  ...
  async function save() {
    ...
    setSaving(true)
    const payload = { title: form.title.trim(), cta_label: form.cta_label.trim() || null, cta_url: form.cta_url.trim() || null, image_url: form.image_url.trim() || null, layout: form.layout, sort_order: Number(form.sort_order) || 0, is_active: form.is_active, updated_at: new Date().toISOString() }
    const result = editing ? await tryGetSupabase().from('lifestyle_cards').update(payload).eq('id', editing.id) : await tryGetSupabase().from('lifestyle_cards').insert(payload)
    setSaving(false)
    if (result.error) return setError(result.error.message)
    toast.success(editing ? 'Lifestyle card updated' : 'Lifestyle card created')
    setModalOpen(false)
    await refresh()
    await refetchCms()
  }
  ...
  async function bulkSetActive(is_active: boolean) {
    ...
    const { error: updateError } = await tryGetSupabase()
      .from('lifestyle_cards')
      .update({ is_active, updated_at: new Date().toISOString() })
      .in('id', bulk.selectedIds)
    setBulkBusy(false)
    if (updateError) return setError(updateError.message)
    toast.success(is_active ? 'Lifestyle cards activated' : 'Lifestyle cards deactivated')
    bulk.clear()
    await refresh()
    await refetchCms()
  }
  ...
  return (
    <div>
          ...
          <table className="w-full min-w-[560px] text-left text-sm">
            <thead className="bg-[var(--admin-surface)] text-[var(--admin-muted)]">
              <tr>
                ...
                <th className="px-4 py-3">Title</th><th className="px-4 py-3">Layout</th><th className="px-4 py-3">Active</th><th className={adminTableActionsHeadClass}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {pageRows.map((row) => (
                <AdminClickableTableRow key={row.id} onOpen={() => setDetail(row)}>
                  ...
                  <td className="px-4 py-3">{row.layout}</td>
      ...
      <AdminSheet open={modalOpen} onOpenChange={setModalOpen} title={editing ? 'Edit lifestyle card' : 'New lifestyle card'} onSave={() => void save()} saving={saving}>
        <div className="grid gap-4">
          <div className="space-y-2"><label className={adminLabel}>Title</label><input className={adminInput} value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} /></div>
          ...
          <div className="space-y-2"><label className={adminLabel}>Layout</label><BrandedSelect value={form.layout} onValueChange={(layout) => setForm((f) => ({ ...f, layout: layout as Layout }))} options={[{ value: 'small', label: 'Small' }, { value: 'large', label: 'Large' }, { value: 'wide', label: 'Wide' }]} /></div>
          <ImageUploadField label="Image" value={form.image_url} onChange={(url) => setForm((f) => ({ ...f, image_url: url }))} folder="lifestyle" />
      ...
      <EntityDetailSheet open={!!detail} onOpenChange={(o) => !o && setDetail(null)} title={detail?.title ?? ''} fields={detail ? [{ label: 'Layout', value: detail.layout }, { label: 'CTA', value: `${detail.cta_label} → ${detail.cta_url}` }] : []} />
    </div>
  )
}
