...
export function AdminProducts() {
  ...
  const [propertyFields, setPropertyFields] = useState<PropertyFieldsForm>(emptyPropertyFields())
  const [propertyLookups, setPropertyLookups] = useState<Awaited<ReturnType<typeof fetchPropertyLookups>> | null>(null)
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const lookups = await fetchPropertyLookups()
      setPropertyLookups(lookups)
    } catch (err) {
  ...
  function openCreate() {
    setEditing(null)
    setForm(emptyForm())
    setPropertyFields(emptyPropertyFields())
    setFormStep(0)
    setModalOpen(true)
  }
  ...
  async function save() {
    ...
    const payload = {
      ...
      updated_at: new Date().toISOString(),
      ...propertyFieldsToPayload(propertyFields),
    }
    ...
    try {
      ...
      if (!productId) throw new Error('Property ID missing after save')
      await syncVariants(productId)
      if (propertyFields.listing_type) {
        await syncPropertyAmenities(productId, propertyFields.amenity_ids)
      }
  ...
  const formSteps = useMemo(
    () => [
      ...
      },
      {
        id: 'property',
        label: 'Property',
        content: propertyLookups ? (
          <PropertyFieldsEditor
            form={propertyFields}
            lookups={propertyLookups}
            onChange={(patch) => setPropertyFields((f) => ({ ...f, ...patch }))}
          />
        ) : (
          <p className="text-sm text-[var(--admin-muted)]">Loading property lookups…</p>
        ),
      },
      {
        id: 'pricing',
