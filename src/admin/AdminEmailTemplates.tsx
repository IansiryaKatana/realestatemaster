...
import { resolveEmailBrandColor } from '@/lib/branding/resolveBrandTheme'
...
const BRAND_KEYS = ['email_footer_text', 'email_from_name', 'store_url'] as const
...
export function AdminEmailTemplates() {
  ...
  const brand: EmailBrandContext = useMemo(() => {
    ...
    return {
      ...
      brandColor: resolveEmailBrandColor({ ...snapshot.siteSettings, ...Object.fromEntries(entries.map((e) => [e.key, e.value])) }),
  ...
  const refresh = useCallback(async () => {
    ...
    const [templatesRes, settingsRes] = await Promise.all([
      ...
      sb.from('site_settings').select('key, value').in('key', [...BRAND_KEYS, 'site_name', 'logo_light_url', 'brand_color_cta_brown']),
    ])
    ...
    <div className="space-y-6">
      ...
      <section className="admin-section space-y-4">
        ...
        <p className="mb-4 text-sm text-[var(--admin-text-muted)]">
          Logo uses your light-background logo from Site Settings. Email header color follows CTA brown from Brand colors in
          Site Settings.
        </p>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className={adminLabel}>Email header color</label>
            <div className="flex items-center gap-3">
              <span
                className="inline-block h-10 w-14 rounded border border-[var(--admin-border)]"
                style={{ backgroundColor: brand.brandColor }}
                aria-hidden
              />
              <p className="text-sm text-[var(--admin-muted)]">
                Uses <strong>CTA brown</strong> from Site Settings → Brand colors ({brand.brandColor})
              </p>
            </div>
          </div>
          <div>
