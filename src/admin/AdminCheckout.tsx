...
export function AdminCheckout() {
  ...
  async function save() {
    if (mode === 'quote' && !quoteEmail.trim()) {
      toast.error('Inquiry notification email is required when quote mode is enabled')
      return
    }
    const sb = tryGetSupabase()
    if (!sb) return
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-semibold">Payment collection mode</h2>
            <p className="text-sm text-[var(--admin-muted)]">Controls how clients pay rent, deposits, and fees online</p>
          </div>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-[var(--admin-border)] p-4 has-[:checked]:border-[var(--admin-primary)]">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--admin-muted)]">
                Clients submit a payment summary for your team to follow up — no online payment at checkout. Use for inquiries before contract approval.
              </p>
            </div>
          </label>
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-[var(--admin-border)] p-4 has-[:checked]:border-[var(--admin-primary)]">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--admin-muted)]">
                Redirect clients to Stripe for rent, deposits, commissions, and other charges. Configure keys under Stripe.
              </p>
            </div>
          </label>
        </div>
        ...
        {mode === 'quote' && (
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="space-y-2">
              <label className={adminLabel}>Quote / inquiry notification email</label>
              <input
                type="email"
              ...
              <p className="text-xs text-[var(--admin-muted)]">
                Payment summaries are sent here when a client requests a quote. Set <code className="rounded bg-[var(--admin-primary-muted)] px-1">RESEND_API_KEY</code> on your Supabase project to enable email delivery.
              </p>
            </div>

            <div className="space-y-2">
              <label className={adminLabel}>Property inquiry notification email</label>
              <input
                type="email"
                className={adminInput}
                value={contactEmail}
              ...
              />
              <p className="text-xs text-[var(--admin-muted)]">
                Property contact form submissions are sent to this address. Configure the contact form template under Email templates.
              </p>
            </div>
          </div>
        )}
