    ...
    <div>
      <AdminTabToolbar actions={
        <>
          ...
          <button type="button" className={adminBtnPrimary} onClick={openCreate}><Plus className="h-4 w-4" />Add promotion</button>
        </>
      } />
      <AdminErrorBanner message={error} />
      <p className="mb-4 text-sm text-[var(--admin-muted)]">
        Optional discount codes for payment summaries — for example agency fee reductions or seasonal promotions on listed properties.
      </p>
      ...
      <AdminSheet open={modalOpen} onOpenChange={setModalOpen} title={editing ? 'Edit promotion' : 'New promotion'} onSave={() => void save()} saving={saving}>
