...
export function resolveStorefrontListParams(
  ...
  if (slug === 'rentals' || slug === 'rent') return { filter: 'rent' }
  if (slug === 'sales' || slug === 'sale') return { filter: 'sale' }
...
function filterStaticProducts(params: StorefrontListParams, snapshot: CmsSnapshot): Product[] {
  ...
  switch (params.filter) {
    ...
    case 'deals':
      ...
      break
    case 'rent':
      products = products.filter((p) => p.listingType === 'rent')
      break
    case 'sale':
      products = products.filter((p) => p.listingType === 'sale')
      break
    case 'category': {
