...
export function mapProductRow(row: Database['public']['Tables']['products']['Row']): Product {
  return {
    ...
    sortOrder: row.sort_order,
    listingType: (row.listing_type as 'rent' | 'sale' | null) ?? null,
    bedrooms: row.bedrooms,
    bathrooms: row.bathrooms,
    sizeSqft: row.size_sqft != null ? Number(row.size_sqft) : null,
    propertyReference: row.property_reference ?? null,
    areaId: row.area_id,
    propertyTypeId: row.property_type_id,
    propertyStatusId: row.property_status_id,
    furnishingStatusId: row.furnishing_status_id,
    ...
    contractTerms: row.contract_terms ?? null,
    agentCommissionType: row.agent_commission_type ?? null,
    agentCommissionValue: row.agent_commission_value != null ? Number(row.agent_commission_value) : null,
    rentPaymentCheques: row.rent_payment_cheques ?? null,
  }
}
