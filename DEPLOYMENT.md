...
Set these in **Netlify → Site configuration → Environment variables**, **Vercel → Project Settings → Environment Variables**, or **Hostinger hPanel → Node app → Environment**:
...
4. Add `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` in **Project Settings → Environment Variables** (Production, Preview, and Development).
5. Deploy.
...
The Nitro `vercel` preset wires SSR and client routes (`/product/...`, `/admin/...`, etc.) to Vercel Functions. No manual rewrites file is required.

### Local Windows builds

`npm run build:vercel` may fail on Windows with `EISDIR: illegal operation on a directory, readlink` when the project lives on a drive other than `C:` (e.g. `F:`). This is a known `@vercel/nft` tracing limitation on Windows — **Vercel’s Linux build servers are unaffected**. Deploy via Git or `vercel --prod` instead of relying on a local Vercel build on Windows.
