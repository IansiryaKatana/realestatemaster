  ...
  try {
    ...
    if (secret_key && webhook_secret) {
      await supabase.from('site_settings').upsert([
        { key: 'stripe_enabled', value: 'true', updated_at: new Date().toISOString() },
        { key: 'checkout_mode', value: 'stripe', updated_at: new Date().toISOString() },
      ])
    }

    return new Response(JSON.stringify({ ok: true, auto_enabled: Boolean(secret_key && webhook_secret) }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  } catch (e) {
