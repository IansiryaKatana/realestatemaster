...
async function fulfillPropertyInvoice(
  ...
  await supabase.rpc('rpc_record_property_payment', {
    p_transaction_id: transactionId,
    p_invoice_id: invoiceId,
  })
}

async function fulfillStoreOrder(
  supabase: ReturnType<typeof createClient>,
  session: Stripe.Checkout.Session,
) {
  ...
  if (orderDetails?.email) {
    ...
    const currency = String(orderDetails.currency ?? 'USD')
    ...
    await sendTemplateEmail(supabase, 'order_confirmation', orderDetails.email, {
      order_number: orderDetails.order_number,
      customer_name: orderDetails.email.split('@')[0] ?? 'Customer',
      customer_email: orderDetails.email,
      order_total: formatMoney(Number(orderDetails.total), currency),
      order_subtotal: formatMoney(Number(orderDetails.subtotal), currency),
      order_date: new Date(orderDetails.created_at).toLocaleDateString('en-US', { dateStyle: 'long' }),
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (session.metadata?.type === 'property_invoice') {
        await fulfillPropertyInvoice(supabase, session)
      } else {
        await fulfillStoreOrder(supabase, session)
      }
    }
